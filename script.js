...
(function() {
    ...
    async function checkSession() {
        ...
        state.user = user;
        try {
            const resumo = await api.getResumo(user);
            state.resumo = resumo;
            // Busca dados do usuário através do resumo ou endpoint de login fictício
            // Como não há endpoint de perfil, usamos o nome vindo do resumo ou login
            // Tentamos obter nome via login (não ideal, mas funcional)
            // Na prática, armazenamos nome no login
            return true;
        } catch (err) {
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            ${pageItems.map(tx => {
                                ...
                                const statusText = isPast ? t('efetuada') : t('pendente');
                                const statusBadge = isPast ? 'badge-success' : 'badge-danger';
                                return `
                                    <tr>
                ...
                ${meta ? `
                    <div class="card" style="margin-bottom:20px;">
                        ...
                        <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:12px;">
                            ...
                            <div><strong>${t('totalGuardado')}:</strong> ${formatCurrency(totalGuardado)}</div>
                            <div><strong>${t('progresso')}:</strong> ${metaTotal > 0 ? Math.round((totalGuardado / metaTotal) * 100) : 0}%</div>
                        </div>
    ...
    window.openTransactionModal = (id = null) => {
        // Implementar modal de transação
        showToast(id ? 'Editar transação' : 'Nova transação', 'info');
        // Placeholder - modal será implementado em versão futura
    };
    ...
    window.toggleCofrinho = (index) => {
        ...
        if (meta) {
            ...
            const grid = document.getElementById('cofrinho-grid');
            if (grid) {
                const parent = grid.closest('.card');
                const summary = parent.querySelector('.cofrinho-summary');
                if (summary) {
                    summary.innerHTML = `
                        <strong>${t('totalGuardado')}:</strong> ${formatCurrency(guardado)}
                        <strong style="margin-left:16px;">${t('progresso')}:</strong> ${Math.round((guardado / metaTotal) * 100)}%
                    `;
                }
            }
        }
